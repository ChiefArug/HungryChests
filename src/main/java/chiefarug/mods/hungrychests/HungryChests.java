...
public class HungryChests {
	...
	private static void serverStart(ServerStartedEvent event) {
		chestHungrifier = event.getServer().registryAccess().registry(Registries.PROCESSOR_LIST).get().getHolder(ResourceKey.create(Registries.PROCESSOR_LIST, processorLocation)).get();
	}
